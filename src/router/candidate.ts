...
function init() {
    ...
    router.get("/:id", async (req, res) => res.send(await candidateDB.getWithId(parseInt(req.params.id))));
    router.put("/:id", async (req, res) => res.send(await candidateDB.update(parseInt(req.params.id), req.body)));
    router.delete("/:id", async (req, res) => res.send(await candidateDB.delete(parseInt(req.params.id))));
    router.get("/cv/:filename", async (req, res) => {
        const filename = req.params.filename
        res.download(process.env.OS === "Linux"
    ...
    router.post("/new", async (req, res) => {
        ...
        if(req.files === null || req.files === undefined) {
            res.status(400).json(req.files === null ? "Null Stuff" : "Undefined stuff")
        } else {
